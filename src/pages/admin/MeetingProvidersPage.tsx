...
export function MeetingProvidersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) {
      await meetingProviderService.update(editing.id, input);
    } else {
      await meetingProviderService.create(input);
    }
  ...
  const handleDelete = async (id: string) => {
    await meetingProviderService.delete(id);
    void fetchProviders();
  };

  const handleToggle = async (id: string, isActive: boolean) => {
    await meetingProviderService.toggleActive(id, isActive);
    void fetchProviders();
  };
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setForm((prev) => ({ ...prev, providerType: e.target.value as MeetingProviderType }))}
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Input label="API Key (optional)" value={form.apiKey} onChange={(e) => setForm((prev) => ({ ...prev, apiKey: e.target.value }))} />
                <Input label="API Secret (optional)" type="password" value={form.apiSecret} onChange={(e) => setForm((prev) => ({ ...prev, apiSecret: e.target.value }))} />
              </div>
              <Input label="Server URL (optional)" value={form.serverUrl} onChange={(e) => setForm((prev) => ({ ...prev, serverUrl: e.target.value }))} />
              ...
              <Card key={p.id} hover>
                <CardContent className="p-4">
                  ...
                  <div className="mt-3 flex items-center gap-2 border-t border-neutral-100 pt-3">
                    <button onClick={() => handleToggle(p.id, !p.isActive)} className="rounded-md px-2 py-1 text-xs text-neutral-600 hover:bg-neutral-100">
                      {p.isActive ? 'Deactivate' : 'Activate'}
                    </button>
                    <button onClick={() => handleEdit(p)} className="rounded-md p-1 text-primary-600 hover:bg-primary-50" aria-label="Edit"><Edit className="h-3.5 w-3.5" /></button>
                    <button onClick={() => handleDelete(p.id)} className="rounded-md p-1 text-error-500 hover:bg-error-50" aria-label="Delete"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </CardContent>
              </Card>
            );
