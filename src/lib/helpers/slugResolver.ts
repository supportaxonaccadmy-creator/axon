...
export const slugResolver = {
  generate(text: string): string { return generateSlug(text); },
  normalize(slug: string): string { return normalizeSlug(slug); },
  ...
  async checkUnique(table: string, slug: string, parentIdColumn?: string, parentId?: string): Promise<boolean> {
    ...
    if (parentIdColumn && parentId) { query = query.eq(parentIdColumn, parentId); }
    const { data, error } = await query.maybeSingle();
    if (error) { logger.error('slugResolver.checkUnique', { error: error.message }); return false; }
    return data === null;
  },
  ...
  async resolve(table: string, text: string, parentIdColumn?: string, parentId?: string): Promise<SlugResolutionResult> {
    ...
    if (isUnique) return { slug, isUnique: true, hadConflict: false };
    let counter = 2;
    let candidate = `${slug}-${counter}`;
    while (!(await this.checkUnique(table, candidate, parentIdColumn, parentId))) { counter += 1; candidate = `${slug}-${counter}`; }
  ...
  async resolveMany(table: string, texts: string[], parentIdColumn?: string, parentId?: string): Promise<SlugResolutionResult[]> {
    ...
    for (const text of texts) {
      ...
      if (usedSlugs.includes(slug)) {
        let counter = 2;
        while (usedSlugs.includes(`${slug}-${counter}`)) { counter += 1; }
        slug = `${slug}-${counter}`;
      }
      const isUnique = await this.checkUnique(table, slug, parentIdColumn, parentId);
      usedSlugs.push(slug);
