...
function PerformanceCardComponent({
  learningScore, engagementScore, consistencyScore, mcqAccuracy, completionPercentage,
        ...
        {metrics.map((m) => (
          <div key={m.label}>
            <div className="mb-1 flex items-center justify-between">
              ...
              <span className={cn('text-sm font-bold', m.color)}>{m.value.toFixed(1)}%</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-neutral-100">
              <div className={cn('h-full rounded-full transition-all', m.color.replace('text-', 'bg-'))} style={{ width: `${Math.min(m.value, 100)}%` }} />
            </div>
          </div>
        ))}
      </div>
