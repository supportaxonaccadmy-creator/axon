...
export const attendanceAnalyticsService = {
  async getOverview(): Promise<{ data: AttendanceAnalyticsData | null; error: string | null }> {
    ...
    try {
      ...
      const statusCounts = [
        { status: 'Present', count: presentCount, percentage: total > 0 ? (presentCount / total) * 100 : 0 },
        { status: 'Absent', count: absentCount, percentage: total > 0 ? (absentCount / total) * 100 : 0 },
        { status: 'Late', count: lateCount, percentage: total > 0 ? (lateCount / total) * 100 : 0 },
      ];

      const trendMap = new Map<string, { present: number; absent: number; late: number }>();
      for (const a of attendanceRows) {
        const date = (a.created_at as string).split('T')[0] ?? '';
        if (!trendMap.has(date)) trendMap.set(date, { present: 0, absent: 0, late: 0 });
      ...
      return {
        data: {
          ...
          attendanceTrend: Array.from(trendMap.entries())
      ...
      logger.error('attendanceAnalyticsService.getOverview', { error: err instanceof Error ? err.message : 'Unknown' });
