...
export const engagementService = {
  async getByStudent(studentId: string, days = 30): Promise<{ data: EngagementMetric[]; error: string | null }> {
    ...
    if (error) {
      logger.error('engagementService.getByStudent', { error: error.message });
  ...
  async getByBatch(batchId: string): Promise<{ data: EngagementMetric[]; error: string | null }> {
    ...
    const { data, error } = await supabase
      ...
      .order('metric_date', { ascending: true });
    if (error) {
      logger.error('engagementService.getByBatch', { error: error.message });
      return { data: [], error: error.message };
    }
    return { data: (data as never[]).map(mapEngagement), error: null };
  },
  ...
  async record(input: {
    ...
    const score = calculateEngagementScore({
      ...
      videosWatched: input.videosWatched ?? 0,
      pdfsRead: input.pdfsRead ?? 0,
      mcqsAttempted: input.mcqsAttempted ?? 0,
      liveClassesAttended: input.liveClassesAttended ?? 0,
      interactions: input.interactions ?? 0,
    });
    const row: Record<string, unknown> = {
      student_id: input.studentId,
      metric_date: input.metricDate ?? new Date().toISOString().split('T')[0],
      session_count: input.sessionCount ?? 0,
      total_duration_minutes: input.totalDurationMinutes ?? 0,
      videos_watched: input.videosWatched ?? 0,
      pdfs_read: input.pdfsRead ?? 0,
      mcqs_attempted: input.mcqsAttempted ?? 0,
      live_classes_attended: input.liveClassesAttended ?? 0,
      interactions: input.interactions ?? 0,
      engagement_score: score,
    };
    const { data, error } = await supabase.from(TABLE).upsert(row, { onConflict: 'student_id,metric_date' }).select('*').maybeSingle();
    if (error) {
      logger.error('engagementService.record', { error: error.message });
