...
export const studentAnalyticsService = {
  async getStudentIntelligence(studentId: string): Promise<{ data: StudentIntelligence | null; error: string | null }> {
    ...
    try {
      ...
      const getPred = (type: StudentPrediction['predictionType']) =>
        preds.find((p) => p.predictionType === type);
      ...
      return {
        data: {
          ...
          email: profile.email ?? '',
          learningScore: analytics?.learningScore ?? 0,
          ...
          mcqAccuracy: analytics?.mcqAccuracy ?? 0,
          attendancePercentage: analytics?.attendancePercentage ?? 0,
          retentionStatus: retention?.retentionStatus ?? 'active',
          churnRiskLevel: retention?.churnRiskLevel ?? 'low',
          predictedScore: getPred('expected_score')?.predictedValue ?? 0,
          predictedRank: Math.round(getPred('expected_rank')?.predictedValue ?? 0),
          dropRisk: getPred('drop_risk')?.predictedValue ?? 0,
          strongSubjects: [],
          weakSubjects: [],
          recommendations: [],
        },
  ...
  async getAllIntelligence(): Promise<{ data: StudentIntelligence[]; error: string | null }> {
    ...
    if (error) {
      logger.error('studentAnalyticsService.getAllIntelligence', { error: error.message });
      return { data: [], error: error.message };
    }
    const results: StudentIntelligence[] = [];
    for (const student of students ?? []) {
      const result = await this.getStudentIntelligence(student.id);
