...
export function mapLearningAnalytics(row: LearningAnalyticsRow): StudentLearningAnalytics {
  return {
    ...
    weeklyStudyMinutes: row.weekly_study_minutes,
    monthlyStudyMinutes: row.monthly_study_minutes,
    completionPercentage: Number(row.completion_percentage),
    attendancePercentage: Number(row.attendance_percentage),
    mcqAccuracy: Number(row.mcq_accuracy),
    videoCompletionPercentage: Number(row.video_completion_percentage),
    pdfReadingPercentage: Number(row.pdf_reading_percentage),
    revisionFrequency: Number(row.revision_frequency),
...
export function mapPrediction(row: PredictionRow): StudentPrediction {
  return {
    id: row.id,
    studentId: row.student_id,
    predictionType: row.prediction_type as StudentPrediction['predictionType'],
    predictedValue: Number(row.predicted_value),
    confidence: Number(row.confidence),
    trend: row.trend as PredictionTrend,
    factors: row.factors ?? {},
    notes: row.notes,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
export function mapEngagement(row: EngagementRow): EngagementMetric {
  return {
    ...
    pdfsRead: row.pdfs_read,
    mcqsAttempted: row.mcqs_attempted,
    liveClassesAttended: row.live_classes_attended,
...
export function mapSnapshot(row: SnapshotRow): PerformanceSnapshot {
  return {
    ...
    studentId: row.student_id,
    batchId: row.batch_id,
    period: row.period as PerformanceSnapshot['period'],
...
export function mapRetention(row: RetentionRow): RetentionMetric {
  return {
    ...
    reEngagementScore: Number(row.re_engagement_score),
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}
...
export function calculateEngagementScore(metrics: {
  ...
  const activityScore = Math.min(
    metrics.videosWatched + metrics.pdfsRead + metrics.mcqsAttempted + metrics.liveClassesAttended,
    20,
  ) * 2.5;
...
export function calculateRetentionStatus(daysSinceLastActivity: number): {
  status: 'active' | 'at_risk' | 'dormant' | 'churned';
