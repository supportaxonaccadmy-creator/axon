...
export const batchAnalyticsService = {
  async getBatchSummary(batchId: string): Promise<{ data: BatchAnalyticsSummary | null; error: string | null }> {
    ...
    try {
      ...
      return {
        data: { batchId, totalStudents, activeStudents,
          ...
          churnRate: totalStudents > 0 ? ((churnedCount / totalStudents) * 100) : 0, atRiskCount },
        error: null,
      };
    } catch (err) {
  ...
  async getAllBatchSummaries(): Promise<{ data: BatchAnalyticsSummary[]; error: string | null }> {
    ...
    if (error) { logger.error('batchAnalyticsService.getAllBatchSummaries', { error: error.message }); return { data: [], error: error.message }; }
    const summaries: BatchAnalyticsSummary[] = [];
    for (const batch of batches ?? []) { const result = await this.getBatchSummary(batch.id); if (result.data) summaries.push(result.data); }
