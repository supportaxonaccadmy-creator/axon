...
function mapChapter(row: ChapterRow): Chapter {
  return { id: row.id, subjectId: row.subject_id, title: row.title, slug: row.slug, description: row.description, sortOrder: row.sort_order, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at };
}

function toRow(input: ChapterInsert): Record<string, unknown> {
  return { subject_id: input.subjectId, title: input.title, slug: input.slug, description: input.description ?? null, sort_order: input.sortOrder ?? 0, status: input.status ?? 'draft' };
}
...
export const chapterService = {
  async getById(id: string): Promise<{ data: Chapter | null; error: string | null }> {
    ...
    if (error) { logger.error('chapterService.getById', { error: error.message }); return { data: null, error: error.message }; }
  ...
  async getBySlug(subjectId: string, slug: string): Promise<{ data: Chapter | null; error: string | null }> {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.from(TABLE).select('*').eq('subject_id', subjectId).eq('slug', slug).maybeSingle();
    if (error) { logger.error('chapterService.getBySlug', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapChapter(data as ChapterRow) : null, error: null };
  },
  ...
  async list(options?: ChapterListOptions): Promise<{ data: Chapter[]; error: string | null }> {
    ...
    if (error) { logger.error('chapterService.list', { error: error.message }); return { data: [], error: error.message }; }
    return { data: (data as ChapterRow[]).map(mapChapter), error: null };
  },

  async paginate(page: number = 1, pageSize: number = 10, options?: ChapterListOptions): Promise<PaginatedResult<Chapter>> {
    const supabase = getSupabaseClient();
    ...
    const { data, error } = await query;
    if (error) { logger.error('chapterService.paginate', { error: error.message }); return { data: [], total: 0, page, pageSize, totalPages: 0, hasNext: false, hasPrev: false }; }
    return { data: (data as ChapterRow[]).map(mapChapter), total, page, pageSize, totalPages, hasNext: page < totalPages, hasPrev: page > 1 };
  },
  ...
  async create(input: ChapterInsert): Promise<{ data: Chapter | null; error: string | null }> {
    ...
    if (error) { logger.error('chapterService.create', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapChapter(data as ChapterRow) : null, error: null };
  },
  ...
  async update(id: string, input: ChapterUpdate): Promise<{ data: Chapter | null; error: string | null }> {
    ...
    if (error) { logger.error('chapterService.update', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapChapter(data as ChapterRow) : null, error: null };
  },
  ...
  async remove(id: string): Promise<{ error: string | null }> {
    ...
    if (error) { logger.error('chapterService.remove', { error: error.message }); return { error: error.message }; }
    return { error: null };
  },
};
