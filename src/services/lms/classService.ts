...
function mapClass(row: ClassRow): Class {
  return { id: row.id, chapterId: row.chapter_id, title: row.title, slug: row.slug, description: row.description, thumbnail: row.thumbnail, duration: row.duration, sortOrder: row.sort_order, isPreview: row.is_preview, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at };
}

function toRow(input: ClassInsert): Record<string, unknown> {
  return { chapter_id: input.chapterId, title: input.title, slug: input.slug, description: input.description ?? null, thumbnail: input.thumbnail ?? null, duration: input.duration ?? null, sort_order: input.sortOrder ?? 0, is_preview: input.isPreview ?? false, status: input.status ?? 'draft' };
}
...
export const classService = {
  async getById(id: string): Promise<{ data: Class | null; error: string | null }> {
    ...
    if (error) { logger.error('classService.getById', { error: error.message }); return { data: null, error: error.message }; }
  ...
  async getBySlug(chapterId: string, slug: string): Promise<{ data: Class | null; error: string | null }> {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.from(TABLE).select('*').eq('chapter_id', chapterId).eq('slug', slug).maybeSingle();
    if (error) { logger.error('classService.getBySlug', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapClass(data as ClassRow) : null, error: null };
  },
  ...
  async list(options?: ClassListOptions): Promise<{ data: Class[]; error: string | null }> {
    ...
    if (error) { logger.error('classService.list', { error: error.message }); return { data: [], error: error.message }; }
    return { data: (data as ClassRow[]).map(mapClass), error: null };
  },

  async paginate(page: number = 1, pageSize: number = 10, options?: ClassListOptions): Promise<PaginatedResult<Class>> {
    const supabase = getSupabaseClient();
    ...
    const { data, error } = await query;
    if (error) { logger.error('classService.paginate', { error: error.message }); return { data: [], total: 0, page, pageSize, totalPages: 0, hasNext: false, hasPrev: false }; }
    return { data: (data as ClassRow[]).map(mapClass), total, page, pageSize, totalPages, hasNext: page < totalPages, hasPrev: page > 1 };
  },
  ...
  async create(input: ClassInsert): Promise<{ data: Class | null; error: string | null }> {
    ...
    if (error) { logger.error('classService.create', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapClass(data as ClassRow) : null, error: null };
  },
  ...
  async update(id: string, input: ClassUpdate): Promise<{ data: Class | null; error: string | null }> {
    ...
    if (error) { logger.error('classService.update', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapClass(data as ClassRow) : null, error: null };
  },
  ...
  async remove(id: string): Promise<{ error: string | null }> {
    ...
    if (error) { logger.error('classService.remove', { error: error.message }); return { error: error.message }; }
    return { error: null };
  },
};
