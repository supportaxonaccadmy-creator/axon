...
function mapSubject(row: SubjectRow): Subject {
  return { id: row.id, batchId: row.batch_id, title: row.title, slug: row.slug, description: row.description, icon: row.icon, sortOrder: row.sort_order, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at };
}

function toRow(input: SubjectInsert): Record<string, unknown> {
  return { batch_id: input.batchId, title: input.title, slug: input.slug, description: input.description ?? null, icon: input.icon ?? null, sort_order: input.sortOrder ?? 0, status: input.status ?? 'draft' };
}
...
export const subjectService = {
  async getById(id: string): Promise<{ data: Subject | null; error: string | null }> {
    ...
    if (error) { logger.error('subjectService.getById', { error: error.message }); return { data: null, error: error.message }; }
  ...
  async getBySlug(batchId: string, slug: string): Promise<{ data: Subject | null; error: string | null }> {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.from(TABLE).select('*').eq('batch_id', batchId).eq('slug', slug).maybeSingle();
    if (error) { logger.error('subjectService.getBySlug', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapSubject(data as SubjectRow) : null, error: null };
  },
  ...
  async list(options?: SubjectListOptions): Promise<{ data: Subject[]; error: string | null }> {
    ...
    if (error) { logger.error('subjectService.list', { error: error.message }); return { data: [], error: error.message }; }
    return { data: (data as SubjectRow[]).map(mapSubject), error: null };
  },

  async paginate(page: number = 1, pageSize: number = 10, options?: SubjectListOptions): Promise<PaginatedResult<Subject>> {
    const supabase = getSupabaseClient();
    ...
    const { data, error } = await query;
    if (error) { logger.error('subjectService.paginate', { error: error.message }); return { data: [], total: 0, page, pageSize, totalPages: 0, hasNext: false, hasPrev: false }; }
    return { data: (data as SubjectRow[]).map(mapSubject), total, page, pageSize, totalPages, hasNext: page < totalPages, hasPrev: page > 1 };
  },
  ...
  async create(input: SubjectInsert): Promise<{ data: Subject | null; error: string | null }> {
    ...
    if (error) { logger.error('subjectService.create', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapSubject(data as SubjectRow) : null, error: null };
  },
  ...
  async update(id: string, input: SubjectUpdate): Promise<{ data: Subject | null; error: string | null }> {
    ...
    if (error) { logger.error('subjectService.update', { error: error.message }); return { data: null, error: error.message }; }
    return { data: data ? mapSubject(data as SubjectRow) : null, error: null };
  },
  ...
  async remove(id: string): Promise<{ error: string | null }> {
    ...
    if (error) { logger.error('subjectService.remove', { error: error.message }); return { error: error.message }; }
    return { error: null };
  },
};
