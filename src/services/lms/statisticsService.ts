...
export const statisticsService = {
  async getBatchStatistics(batchId: string): Promise<{ data: BatchStatistics | null; error: string | null }> {
    ...
    try {
      ...
      return { data: { subjectCount: subjects.count ?? 0, chapterCount: chapters.count ?? 0, classCount: classes.count ?? 0, videoCount: videos.count ?? 0, pdfCount: pdfs.count ?? 0, mcqCount: mcqSets.count ?? 0, enrollmentCount: enrollments.count ?? 0, revenue }, error: null };
    } catch (err) {
  ...
  async getGlobalStatistics(): Promise<{ data: GlobalStatistics | null; error: string | null }> {
    ...
    try {
      ...
      const publishedCounts = await Promise.all(tables.slice(0, 9).map((t) => supabase.from(t).select('id', { count: 'exact', head: true }).eq('status', 'published')));
      const draftCounts = await Promise.all(tables.slice(0, 9).map((t) => supabase.from(t).select('id', { count: 'exact', head: true }).eq('status', 'draft')));
      ...
      return { data: { batchCount: countMap['batches'] ?? 0, subjectCount: countMap['subjects'] ?? 0, chapterCount: countMap['chapters'] ?? 0, classCount: countMap['classes'] ?? 0, videoCount: countMap['videos'] ?? 0, pdfCount: countMap['pdf_notes'] ?? 0, mcqSetCount: countMap['mcq_sets'] ?? 0, mcqQuestionCount: countMap['mcq_questions'] ?? 0, attachmentCount: countMap['attachments'] ?? 0, enrollmentCount: countMap['enrollments'] ?? 0, purchaseCount: countMap['purchases'] ?? 0, totalRevenue, publishedCount, draftCount, archivedCount: 0 }, error: null };
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown error';
  ...
  async getContentStatusCounts(table: string): Promise<{ data: ContentStatusCounts | null; error: string | null }> {
    ...
    try {
      ...
      return { data: { published: published.count ?? 0, draft: draft.count ?? 0, archived: archived.count ?? 0 }, error: null };
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown error';
  ...
  async getRevenueSummary(): Promise<{ data: { totalRevenue: number; completedCount: number; pendingCount: number; failedCount: number; refundedCount: number } | null; error: string | null }> {
    ...
    try {
      ...
      return { data: { totalRevenue, completedCount: completed.data?.length ?? 0, pendingCount: pending.count ?? 0, failedCount: failed.count ?? 0, refundedCount: refunded.count ?? 0 }, error: null };
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown error';
